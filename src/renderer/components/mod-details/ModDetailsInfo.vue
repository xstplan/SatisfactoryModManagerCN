  ...
  >
    ...
    >
        ...
        >
          <template #activator="{ on, attrs }">
            <v-btn
              ...
              v-on="on"
            >
              <span>其他贡献者 <span class="primary--text">({{ mod.authors.length }})</span></span>
              <v-spacer />
              <v-icon
                right
                dark
      ...
      <v-row style="padding-bottom: 10px">
        <span>
          Mod 信息:<br>
          大小: <span class="header">{{ mod.versions[0] && mod.versions[0].size ? bytesToAppropriate(mod.versions[0].size) : 'N/A' }}</span><br>
          创建: <span class="header">{{ mod.created_at.toLocaleDateString() }}</span><br>
          更新: <span class="header">{{ mod.last_version_date ? mod.last_version_date.toLocaleString() : 'N/A' }}</span><br>
          当前版本: <span class="header">{{ mod.versions[0] ? mod.versions[0].version : 'N/A' }}</span><br>
        </span>
      </v-row>
      <v-row
        style="padding-bottom: 10px"
      ...
      <v-row style="padding-bottom: 10px">
        <span>
          ...
          总下载量: <span class="header">{{ mod.downloads.toLocaleString() }}</span><br>
          查看: <span class="header">{{ mod.views.toLocaleString() }}</span><br>
          <!--Tags:<br>
