  ...
  >
    <v-card color="loadingBackground !important">
      ...
      <v-card-text v-if="!allLeftover && updateReminder">
        Remember to <a @click="disableOutdatedMods(true)">检查更新</a>, 并单击更新菜单中的“全部更新”，而不是更新单个MOD.
      </v-card-text>

      <v-card-text v-else-if="!allLeftover">
        你的一些MOD已经过时了。是否要禁用它们？
        <ul>
          <li
      ...
      <v-card-text v-else>
        无法禁用某些MOD。它们可能是依赖项，您应该 <a @click="disableOutdatedMods(true)">检查更新</a>.
        <ul>
          <li
            v-for="mod in outdatedInstalledMods"
            :key="mod.modReference"
      ...
      <v-card-actions>
        ...
        >
          <span v-if="!updateReminder">禁用过时的MOD&amp; 更新</span>
          <span v-else>更新 mods</span>
        </v-btn>
        <v-btn
          color="text"
          text
