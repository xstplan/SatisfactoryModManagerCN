      ...
      >
        ...
        <template v-if="(!launchButton && !launchCat) || !(selectedInstall && selectedInstall.launchPath && !isGameRunning)">
          ...
          >
            ...
            >
              您有{{ possiblyCompatibleInstalledCount }} mod{{ possiblyCompatibleInstalledCount > 1 ? 's' : '' }}
              that {{ possiblyCompatibleInstalledCount > 1 ? 'are' : 'is' }} likely incompatible and
              {{ incompatibleInstalledCount }} mod{{ incompatibleInstalledCount > 1 ? 's' : '' }} 与您的游戏不兼容。
              有可能不会加载MOD或者会崩溃您的游戏。您确定要启动吗？
            </span>
            <span v-else-if="incompatibleInstalledCount !== 0">
              您有 {{ incompatibleInstalledCount }} 不兼容 mod{{ incompatibleInstalledCount > 1 ? 's' : '' }} 有可能不会加载MOD或者会崩溃您的游戏。您确定要启动吗?
            </span>
            <span v-else>
              您有 {{ possiblyCompatibleInstalledCount }} mod{{ possiblyCompatibleInstalledCount > 1 ? 's' : '' }}
              that {{ possiblyCompatibleInstalledCount > 1 ? 'are' : 'is' }} 可能不兼容，可能会使游戏崩溃。您确定要启动吗？?
            </span>
          </v-tooltip>
        </template>
