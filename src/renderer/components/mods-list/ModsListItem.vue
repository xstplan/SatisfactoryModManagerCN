    ...
    >
      ...
      >
        ...
        >
          ...
          <span v-if="disabled">此模块已禁用。按暂停图标以启用它.</span>
        </v-tooltip>
        ...
        <template v-if="(isEnabled || isInstalled) && !isModInProgress">
          ...
          >
            <template #tooltip>
              ...
              <span v-else-if="!modsEnabled">使mods能够进行更改</span>
              <span v-else-if="isGameRunning">游戏运行时无法安装mods</span>
              <span v-else-if="inProgress.length > 0">另一项行动正在进行中</span>
            </template>
          </ModActionButton>
        </template>
      </v-list-item-action>
  ...
  asyncComputed: {
    ...
    errorTooltip: {
      get() {
        ...
        if (this.isPossiblyCompatible) return '此mod可能与您的游戏版本不兼容，并可能导致崩溃.';
        return '此mod与您的游戏版本不兼容.';
      },
      default: null,
    },
    dependantsFriendly: {
