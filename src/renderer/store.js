...
export default new Vuex.Store({
  ...
  actions: {
    async selectInstall({ commit, dispatch, state }, newInstall) {
      ...
      if (!state.inProgress.some((prog) => prog.id === '__loadingApp__')) {
        const loadProgress = {
          id: '__loadingApp__',
          progresses: [{
            id: '', progress: -1, message: '正在验证mod安装', fast: false,
          }],
        };
    ...
    async installMod({
      ...
      if (!state.manifestItems.some((item) => item.id === modId)) {
        if (state.inProgress.length > 0) {
          dispatch('showError', `尝试安装mod时，另一个操作当前正在进行： ${state.inProgress.map((progress) => progress.id)}`);
          return;
        }
        if (!state.modsEnabled) {
          dispatch('showError', '使mods能够进行更改');
          return;
        }
        ...
        placeholderProgreess.message = '查找要安装的最佳版本';
        try {
    ...
    async uninstallMod({
      ...
      if (state.manifestItems.some((item) => item.id === modId)) {
        if (state.inProgress.length > 0) {
          dispatch('showError', `尝试卸载mod时，另一个操作当前正在进行： ${state.inProgress.map((progress) => progress.id)}`);
          return;
        }
        if (!state.modsEnabled) {
          dispatch('showError', '使mods能够进行更改');
        ...
        modProgress.progresses.push(placeholderProgreess);
        placeholderProgreess.message = '检查不再需要的MOD';
        try {
    ...
    async enableMod({
      ...
      if (state.manifestItems.some((item) => item.id === modId)) {
        if (state.inProgress.length > 0) {
          dispatch('showError', `尝试启用mod时，另一个操作当前正在进行： ${state.inProgress.map((progress) => progress.id)}`);
          return;
        }
        if (!state.modsEnabled) {
          dispatch('showError', '使mods能够进行更改');
    ...
    async disableMod({
      ...
      if (state.installedMods[modId]) {
        if (state.inProgress.length > 0) {
          dispatch('showError', `试图禁用mod时，另一个操作当前正在进行: ${state.inProgress.map((progress) => progress.id)}`);
          return;
        }
        if (!state.modsEnabled) {
          dispatch('showError', '使mods能够进行更改');
    ...
    async installModVersion({
      ...
      if (state.inProgress.length > 0) {
        dispatch('showError', `尝试安装mod版本时，另一个操作当前正在进行: ${state.inProgress.map((progress) => progress.id)}`);
        return;
      }
      if (!state.modsEnabled) {
        dispatch('showError', '使mods能够进行更改');
    ...
    async installSMLVersion({
      ...
      if (state.inProgress.length > 0) {
        dispatch('showError', `尝试安装SML版本时，另一个操作当前正在进行: ${state.inProgress.map((progress) => progress.id)}`);
        return;
      }
      if (!state.modsEnabled) {
        dispatch('showError', '使mods能够进行更改');
    ...
    async initApp({
      ...
      const appLoadProgress = {
        id: '__loadingApp__',
        progresses: [{
          id: '', progress: -1, message: '加载中', fast: false,
        }],
      };
            ...
            if (installs.length === 0) {
              if (invalidInstalls.length !== 0) {
                ...
                if (invalidInstalls.length > 1) {
                  dispatch('showErrorPersistent', new Error(`${invalidInstalls.length} 找到了幸福工厂的安装，但所有安装都指向不存在的文件夹.\n${invalidInstallsString}`));
                } else {
                  dispatch('showErrorPersistent', new Error(`${invalidInstalls.length} 找到幸福工厂的安装，但它指向一个不存在的文件夹.\n${invalidInstallsString}`));
                }
              } else {
                dispatch('showErrorPersistent', new Error('没有找到幸福工厂安装目录.'));
              }
              state.inProgress.remove(appLoadProgress);
              return;
            }
            commit('setInstalls', { installs });
            const installValidateProgress = { id: 'validatingInstall', progress: -1, message: '正在验证mod安装' };
